...
This skill drives a full feature, product, OR non-code deliverable (analysis, presentation, report, Power BI / BI dashboard, data pipeline, document, research write-up) through twelve phases. The product of running it is a working, verified, reviewed, simplified artifact plus a durable living setup at the project root (two docs and one folder) that lets the work survive a `/clear` and resume in any future session:

- `PROJECT.md` - the **living source of truth**, read in full once per session: goal, scope, the Map, current state, decisions locked (rule lines), open questions, key lessons (rule lines). Maintained every session, unprompted. Same format the `/live-document` skill uses, so that skill can curate it too.
- `project-memory/` - the **detail layer**, read on demand via the Map: `decisions.md` (full wording), `lessons.md` (stories), `changelog.md` (milestones), plus two e2e-owned files while a run is active: `research-notes.md` (Phase 4 findings with citations) and `execution-plan.md` (the master phase/step status table and per-step detail; deleted at Phase 12 after its outcome is folded into `changelog.md` / *Current state*).
- `CLAUDE.md` - a **thin constitution / bootstrap** that auto-loads every message, points at PROJECT.md, and carries the `<!-- e2e-state: -->` resume marker. Budget **8 KB / 100 lines** (lint-checked): anything a single message does not need goes to PROJECT.md or `project-memory/` per live-document's *CLAUDE.md routing test*.

**No tracking files outside `project-memory/`.** Discovery output (Office Hours, CEO Review, Elon) is distilled straight into PROJECT.md's canonical sections, research lands in `project-memory/research-notes.md`, the plan lands in `project-memory/execution-plan.md`, and the Phase 8 playtest checklist lives inside the execution plan's Phase 8 detail block. Do not create `PLAN.md`, `RESEARCH.md`, `design-*.md`, `ceo-plan-*.md`, or `PLAYTEST.md` on a fresh run. **Legacy runs** (projects whose earlier `/e2e` version already created those files, or a 2026-09-02-era run holding `## Research notes` / `## Execution plan` inside PROJECT.md) keep their layout for the life of that run - see the resume protocol.
...
**Stand up the living docs at the very start, not at Phase 5.** On a fresh run, create a thin `CLAUDE.md` (carrying the `e2e-state` marker), a `PROJECT.md` skeleton and the `project-memory/` folder in Phase 0, *before* discovery runs - so the user can `/clear` after ANY phase boundary and resume cold. A long discovery+build run must never hold state only in the context window. Every phase from Phase 1 on updates the living docs and bumps the marker. Phase 5 does not *create* these files; it *expands* PROJECT.md to full depth and writes `project-memory/execution-plan.md`.

From Phase 1 onward, PROJECT.md is the single source of truth, maintained unprompted exactly as the `/live-document` skill prescribes. **Every update is a reconcile-and-sweep, never an append.** The living setup has three layers by read frequency (live-document, 2026-09-09): `CLAUDE.md` is read every message (thinnest: max 8 KB / 100 lines, lint-checked, holding only the marker, the Shipped pointer, the workflow contract, Files of note and the live-document block); `PROJECT.md` is read in full once per session (the SessionStart hook only reports its size and the edit gate denies project writes until it was read) under a whole-file budget of 20 KB / 250 lines - header, Goal, Scope, Map, Current state, Decisions locked (rule + one-line why + who/when, max 3 lines), Plan, Open questions, Lessons (key rules, max 3 lines); `project-memory/` holds every detail, read on demand via the Map - `decisions.md` (full wording), `lessons.md` (stories, max 8 lines each), `changelog.md` (every milestone, one entry per date, max 3 lines, no total cap), and the two e2e-owned files `research-notes.md` and `execution-plan.md`. Home rule: a fact lives in ONE home (decision wording / lesson story / milestone in project-memory, rule body in the owning skill's references, run analysis in the run file, owner preference in memory) and appears elsewhere as one line plus a pointer; a line may leave only when its home is named and exists. Write order: project-memory home first, PROJECT.md last. The Map names every folder and file that matters, has one row per project-memory file, and changes in the same edit as any move or archive. On every write:

- Rewrite the touched sections in place, superseding old content: *Current state and next action* describes only NOW; a new durable choice REPLACES the decision it supersedes in `project-memory/decisions.md` (full wording) and in *Decisions locked* (rule + who/when, never stack old and new); an answered *Open question* is deleted, its answer folded into a decision. One fact, one home.
- Feedback and failures go to `project-memory/lessons.md` as the story (deduped: what was tried, what failed, the lesson) and to *Lessons* as a rule line only when they change how we work here.
- Only a milestone (usually a phase boundary) earns a `project-memory/changelog.md` entry (newest first), and an entry is **1-3 lines**: what shipped, the commit, the outcome. Verification narratives, review blow-by-blow, and mechanism detail never go in the log. Most mid-phase updates add no entry.
- Sweep before saving: delete or merge everything now redundant, resolved, stale, or duplicated - anywhere in PROJECT.md and in the project-memory files you touched. Deleting a line that no longer earns its place is REQUIRED maintenance, not data loss (real decisions and lessons are compacted or moved, never dropped). No invented sections in PROJECT.md: only the template's canonical headers (Goal, Scope, Map, Current state, Decisions locked, Plan, Open questions, Lessons), no suffixes, no `## Change log` / `## Research notes` / `## Execution plan` inside it.
- **Red-flag test:** an update that only adds lines and rewrites nothing is almost always wrong. Quantitative tripwires are enforced by the PROJECT.md hooks (`project-md-lint` on every write, the edit gate, a Stop gate that will not end a turn with an unreconciled PROJECT.md, a PreCompact gate): PROJECT.md over 20 KB / 250 lines, a decision over 3 lines, a lesson rule over 3 lines, an open question over 3 lines or partly settled, a migrated section still inside PROJECT.md, a project-memory file without a Map row, a pointer that does not resolve, a Map path that does not exist, a top-level folder missing from the Map, a `lessons.md` story over 8 lines, a `changelog.md` entry over 3 lines or two entries on one date. Each means compaction is overdue and must happen in THIS edit. Numbers live in live-document's SKILL.md and the lint's `LIMITS`; change them together.

One source of truth only: `project-memory/execution-plan.md` tracks *execution state*; PROJECT.md's canonical sections and the other project-memory files hold *everything durable*. Do not duplicate one into the other (legacy runs: the execution state lives in PLAN.md or in a `## Execution plan` section instead - same rule). Write these updates silently, but surface a one-line summary of what changed at each checkpoint.
...
**Self-heal older projects on resume** - four one-time, silent upgrades: (a) if the CLAUDE.md `<!-- live-document:start -->` block carries old-style maintenance items (the phrase "after any answer or change", a "Curate, do not bloat" / "Maintain, don't just append" item, or items 2-4 lacking the word "Tripwire"), replace just items 2-4 with the current items from `references/claude-md-template.md`; (b) if PROJECT.md lacks the `MAINTENANCE CONTRACT` comment, inject it under the title blockquote and add the per-section comments from `references/project-md-template.md`; if its contract lacks the `Tripwire` item, replace the whole comment with the template's; (c) if the block's Hard rules have no bullet containing "Summary, then Reasoning", insert the current chunk-delivery bullet from `references/claude-md-template.md` right after the "Ask before assuming" rule; (d) if the block's Hard rules have no bullet containing "Next Actions", insert the two current bullets (Next Actions file + tidy root) from `references/claude-md-template.md` right after the "Summary, then Reasoning" bullet. (e) if PROJECT.md lacks `## Map - where to find what`, scaffold it after *Scope and non-goals* from the file's own pointers and the project's top-level folders; (f) if the block's items 1-4 or PROJECT.md's contract comment lack the phrase "home rule", replace them with the current ones from the templates; (g) **project-memory (2026-09-09):** if the project has no `project-memory/` folder or the contract comment lacks the phrase "project-memory", migrate at the next phase boundary (never mid-phase) per live-document's `references/project-memory-template.md` - create the folder, move the full decision wording / lesson stories / change log out, move `## Research notes` to `research-notes.md` and `## Execution plan` to `execution-plan.md`, add the Map rows, replace the contract and the block's items 1-4, lint clean. (h) **CLAUDE.md budget (2026-09-18):** if CLAUDE.md is over 8 KB / 100 lines or carries more than 3 KB outside the live-document block, run live-document's *CLAUDE.md slimming* at the next phase boundary (never mid-phase): build the move list with its *CLAUDE.md routing test*, ask ONE yes/no, move each section to its home, repoint references, lint clean. Content you did not write is moved, never deleted and never left fat. Preserve every other line and the markers; **never touch the `e2e-state` marker, and on legacy runs never touch PLAN.md**. Do NOT migrate a legacy multi-file run to the consolidated layout mid-run.
...
2. **Curate the living docs** - reconcile per the Living-document discipline, project-memory first and PROJECT.md last (*Current state* always; decisions superseded in place in both files; answered questions deleted; milestone line in `project-memory/changelog.md`; failures to `project-memory/lessons.md`, rule line to *Lessons* if it changes how we work; sweep).
...
- **`project-memory/`** - the three standard files (`decisions.md`, `lessons.md`, `changelog.md`) from live-document's `references/project-memory-template.md`, each with its standard header and a Map row in PROJECT.md.

Do NOT write `project-memory/execution-plan.md` here - that stays a Phase 5 artifact. Update both files and bump the marker at the end of every phase from Phase 1 on (`office-hours` -> `ceo-review` -> `elon` -> `research` -> `ready-to-execute`). On a **resume**, these files already exist - skip creation and follow the resume protocol.
...
3. **Synthesize** into `project-memory/research-notes.md` using `references/research-template.md` (it carries both track shapes), with its Map row in PROJECT.md. Cite sources with URLs and access dates. No RESEARCH.md at the root - findings that harden into decisions move to *Decisions locked* (rule line) and `project-memory/decisions.md` (full wording, citation); the file keeps only still-load-bearing reference material.
...
- Score each against the discovery + Elon outcomes in PROJECT.md and the `project-memory/research-notes.md` findings.
...
**Expand the living docs and add the execution tracker** (PROJECT.md, CLAUDE.md and the three standard project-memory files already exist from Phase 0):
...
For each step in `project-memory/execution-plan.md` (legacy runs: PLAN.md or the `## Execution plan` section), in order:
...
If a step uncovers something the plan got wrong, **edit the execution plan** (and note it in `project-memory/lessons.md` / `changelog.md`) before moving on. The plan is a living document, not a frozen contract.
...
2. **Hand off to user:** *"Phase 7 green. Please exercise / read the product end to end. Mark each checklist row `OK`, `nit: ...`, `annoying: ...`, or `blocker: ...` - in the file or right here in chat. Bring it back when done."* This hand-off is always a real handoff: also write the Next Actions file (per the Communication format) walking the user through exercising the product and marking the rows.
...
- **Build track - code review.** Spawn a `general-purpose` agent with the code-review lens (superpowers 6.x no longer ships a `code-reviewer` agent; an older install that still offers `superpowers:code-reviewer` may use it): reviews the source against PROJECT.md (goal, decisions locked) and `project-memory/execution-plan.md` for bugs, security issues, design problems, plan deviations, code-quality concerns.
...
If yes, follow `references/playwright-protocol.md`: launch the dev server in the background, drive the browser via the Playwright MCP tools, exercise the golden path from the execution plan's success criteria (`project-memory/execution-plan.md`) plus the protocol's five edge cases, check the accessibility tree on every page, watch console health, report findings, fix what's found and re-run the affected flows.
...
1. Update the CLAUDE.md marker to `<!-- e2e-state: phase=complete track=<T> -->` and fill the "Shipped" placeholder with TWO lines only: the conclusion date and a pointer to PROJECT.md's *Current state and next action*, where what shipped actually lives. CLAUDE.md never carries project state (it auto-loads forever and goes stale).
2. **Retire the execution plan.** Fold its outcome into the living docs (shipped summary + final validation -> a `project-memory/changelog.md` milestone entry and *Current state*; step-level lessons -> `project-memory/lessons.md`), then **delete `project-memory/execution-plan.md` and its Map row**. A completed run's step table is dead weight. (Legacy runs: fill PLAN.md's Outcome paragraph and mark all rows `done` instead - do not delete their PLAN.md; a `## Execution plan` section inside PROJECT.md is deleted the same way.)
3. **Compact `project-memory/research-notes.md`** to only the findings still load-bearing for future work (with citations); delete findings that only justified now-locked decisions (those are already cited in `decisions.md`).
4. Finalize **PROJECT.md** and **project-memory/** with a full curation sweep: *Current state* set to the shipped state and next action (or "complete - no further action"); decisions, changelog, and lessons reflect the whole run; merge duplicates, delete every resolved *Open question*, keep every Map row current. The standing test must hold: a fresh agent reading PROJECT.md (and, on demand, the project-memory files its Map names) can continue without re-explanation.
...
The skill is done. `PROJECT.md`, `CLAUDE.md` and `project-memory/` remain, and they keep maintaining themselves under the `/live-document` bootstrap for the life of the project.

---

## Resume protocol (quick reference)

If invoked in a project where CLAUDE.md contains an e2e-state marker, jump straight to that phase/step. Full details in `references/resume-protocol.md`. The TL;DR: the marker is the source of truth (including `track=`); trust it, read CLAUDE.md + PROJECT.md (legacy runs: also PLAN.md and the old artifact files), read `project-memory/execution-plan.md` and `research-notes.md` when the marker is at Phase 4 or later, summarize what's been done, and continue from where it left off.
...
- **A phase reveals the plan was wrong:** edit `project-memory/execution-plan.md`, note it in `lessons.md` / `changelog.md`, then continue. Don't pretend nothing changed.
