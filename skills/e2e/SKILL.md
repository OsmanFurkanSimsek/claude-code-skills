...
From Phase 1 onward, PROJECT.md is the single source of truth, maintained unprompted exactly as the `/live-document` skill prescribes. **Every update is a reconcile-and-sweep, never an append.** PROJECT.md has two tiers (live-document, 2026-09-02): Tier 1 (header, Goal, Scope, Map - where to find what, Current state, Open questions, Decisions locked) is injected every session by the SessionStart hook under a hard 14 KB / 180-line budget; Tier 2 (Plan, Change log, Lessons, Research notes, Execution plan) is read on demand and may grow within per-entry limits. Home rule: a fact lives in ONE home (lesson story in Lessons, rule body in the owning skill's references, run analysis in the run file, owner preference in memory) and appears elsewhere as one line plus a pointer; a line may leave only when its home is named and exists. The Map names every folder and file that matters and changes in the same edit as any move or archive. On every write:
...
- Sweep the whole file before saving: delete or merge everything now redundant, resolved, stale, or duplicated - anywhere in the file. Deleting a line that no longer earns its place is REQUIRED maintenance, not data loss (real decisions and lessons are compacted or moved, never dropped). No invented sections: only the template's canonical headers (now including `## Map - where to find what`) plus the two e2e-owned sections, no suffixes. Decisions are rule + who/when + pointer, max 4 lines; lessons are the full story, max 8 lines.
- **Red-flag test:** an update that only adds lines and rewrites nothing is almost always wrong. Quantitative tripwires are enforced by the PROJECT.md hooks (`project-md-lint` on every write, a Stop gate that will not end a turn with an unreconciled PROJECT.md, a PreCompact gate): Tier 1 over 14 KB / 180 lines, a decision over 4 lines, a lesson over 8 lines, an open question over 3 lines or partly settled, a Change log over 30 lines, an entry over 3 lines, two entries on one date, an entry older than 14 days still over 1 line, a pointer that does not resolve, a Map path that does not exist, a top-level folder missing from the Map. Each means compaction is overdue and must happen in THIS edit. Numbers live in live-document's SKILL.md and the lint's `LIMITS`; change them together.
