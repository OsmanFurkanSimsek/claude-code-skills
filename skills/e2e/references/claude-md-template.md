...
- **Keep it thin.** Target ~one screen, hard budget 8 KB / 100 lines. CLAUDE.md is auto-loaded into every message, so length costs tokens forever. Route every addition with live-document's *CLAUDE.md routing test*: only what EVERY message needs stays here (as a Hard rules bullet or a one-line pointer to a `project-memory/` file); what every session needs goes to **PROJECT.md**; architecture notes, reference tables, dated notes, run instructions and change logs go to **`project-memory/`**.
...
- **If the project already has a CLAUDE.md when `/e2e` starts**, add the `e2e-state` marker and the workflow contract, and append the live-document block at the end if it isn't already there. Then measure it: if it exceeds 8 KB / 100 lines, or more than 3 KB sits outside the block, route the existing content with live-document's *CLAUDE.md routing test* (per-message rules stay as Hard rules bullets; vision and decisions go to PROJECT.md / `project-memory/decisions.md`; reference material to `project-memory/<topic>.md`), show the move list and ask ONE yes/no before moving anything. Never delete content whose new home does not hold it yet.
- **Self-heal cue (CLAUDE.md budget, 2026-09-18):** a CLAUDE.md over 8 KB / 100 lines, or with more than 3 KB outside the live-document block, gets live-document's *CLAUDE.md slimming* at the next phase boundary (SKILL.md self-heal item (h)).
